  ...
  public static void main(String[] args) throws IOException {
    ...
    Dataset<Row> tableDs = rawDs.filter(col("eventName").equalTo("attribute"))
                               ...
                               .select(col("trackingId").as("game_id"),
                                   ...
                                   col("eventPayload.check.mapId").as("map_id"),
                                   col("eventPayload.check.checkId").as("check_id"),
                                   col("eventPayload.battle.battleTime").as("battle_time"),
                                   col("eventPayload.battle.onlineTime").as("online_time"),
                                   col("eventPayload.battle.isFirst").as("battle_isFirst"),
                                   col("eventPayload.battle.battleResult").as("battle_result"),
                                   col("time").as("time_str"),
                                   col("date").as("date_str"));
  
    tableDs.write().mode(SaveMode.Append).format("orc").insertInto("game_analysis.dwd_user_game_play");
    spark.sql("msck repair table game_analysis.dwd_user_game_play");
  }
}